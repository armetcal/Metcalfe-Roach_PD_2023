...
run_ROC = function(dataset,y,x, name = NA,testsize=0.8,testfamily = 'binomial',weights=F){
  # dataset=temp;y='Status';x=i;name=i;testsize=0.8;testfamily = 'binomial'
  require(ROCR)
  require(PRROC)
  set.seed(421)
  # Divide data
...
prep_plots = function(data,plotvar,plotheights,comparisons=NULL){
  ...
  taxa = indiv_univar$Name[1:ntaxa] # Taxa names
  ...
  p1 = modeldf %>% ggplot(aes(!!sym(plotvar),value,fill = !!sym(plotvar))) +
    ...
    theme_classic(base_size = 18) + 
    xlab(NULL) + ylab('Model Fit') + ggtitle('Taxa') + ggeasy::easy_center_title() +
  ...
  p2 = modeldf %>% ggplot(aes(!!sym(plotvar),value,fill = !!sym(plotvar))) +
    ...
    geom_jitter(height = 0, width = 0.2) +
    theme_classic(base_size = 18) + 
    xlab(NULL) + ylab('Model Fit') + ggtitle('Taxa and Covar') + ggeasy::easy_center_title() +
    ggpubr::stat_compare_means(label = 'p.format', size = 5.5,comparisons=comparisons) +
    theme(legend.position='none') +
    scale_y_continuous(expand = expansion(mult = 0.15))
  ...
  return(list(box=g1,roc=g3,est=g4,comb=g,lfc=c(species=lfc1,cov_sp=lfc2,covar=lfc3)))
}
