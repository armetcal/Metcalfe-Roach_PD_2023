...
run_lmer_multi = function(df,species,variable,log=F,pc = 1, meanscale=T){
  ...
  df$test = rescale(df$test)
  if(meanscale){df$test = df$test-mean(df$test,na.rm=T)}
  if(length(unique(df$ent_boolean))==1){
  a = lmer(test ~ Sex + depth + Abundance*months_last_visit + (months_last_visit|Sample), 
           data = df %>% filter(Species==species), na.action = 'na.omit',REML=F, 
           control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000))) %>% 
    summary %>% .$coefficients %>% as.data.frame() %>% rownames_to_column('Variable') %>% 
    mutate(Taxon = species,Test=variable)
  } else {
    a = lmer(test ~ Sex + ent_boolean + depth + Abundance*months_last_visit + (months_last_visit|Sample), 
             data = df %>% filter(Species==species), na.action = 'na.omit',REML=F, 
             control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000))) %>% 
      summary %>% .$coefficients %>% as.data.frame() %>% rownames_to_column('Variable') %>% 
      mutate(Taxon = species,Test=variable)
...
plot_lmer_faecali = function(df,v,bsize=20){
  # df = allres.p; v=c('mds.total','mds1.total','mds2.total','mds3.total','mds4.total')
  ...
  p = df %>% 
    ...
    mutate(Qval2 = ifelse(qval>0.1,'',
                          ifelse(qval>0.05,'+',''))) %>% 
    ggplot(aes(Test,Group,col = Estimate)) +
    geom_point(size =14) +
    theme_classic(base_size = bsize) +
    scale_color_gradient2(low="darkblue", high="darkred", guide="colorbar") +
    geom_text(aes(label=Qval),size = 12, col = 'white',nudge_y = -0.25) +
...
plot_lmer_faecali_combined = function(df,v,bsize=20){
  # df = input.all; v=c('mds.total','mds1.total','mds2.total','mds3.total','mds4.total'); bsize=20
  ...
  p = df %>% 
    ...
    mutate(Qval2 = ifelse(qval>0.1,'',
                          ifelse(qval>0.05,'+',''))) %>% 
    ggplot(aes(Test,`Stool Firmness`,col = Estimate)) +
    geom_point(size =14) +
    theme_classic(base_size = bsize) +
    scale_color_gradient2(low="darkblue", high="darkred", guide="colorbar") +
    geom_text(aes(label=Qval),size = 12, col = 'white',nudge_y = -0.25) +
