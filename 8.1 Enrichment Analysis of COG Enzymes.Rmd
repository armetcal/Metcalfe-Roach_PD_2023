...
u1 = readxl::read_xlsx('../Final Outputs/DA Results/DA_EGG_Status_All_Stats_3tools.xlsx',sheet='univar') %>% 
  combine_da_results(alpha=pval_cutoff,stat='p_val')
length(unique(u1$Species)) == nrow(u1) # True if no conflicting results

m1 = readxl::read_xlsx('../Final Outputs/DA Results/DA_EGG_Status_All_Stats_3tools.xlsx',sheet='multivar') %>% 
...
length(unique(brite$COG)) == nrow(df) # Some COGs map to multiple pathways, but the unique IDs are identical
```
...
run_fisher = function(level,brite_cat,min_terms=10){
  ...
  for(test_cat in brite_cat){
    ...
    # SelCOGt all COG that are part of the desired pathway
...
```{r Save Results}
writexl::write_xlsx(list('All Results p<0.1' = f.L1, BRITE = brite, all_sig_rxn = allsig),'../Final Outputs/Enrichment Analyses/COG_3Tools_COGonly.xlsx')
```
