---
title: "6e. Disease Severity - Metabolites"
...
other_levels = colnames(ps@tax_table)
psm = ps %>% subset_taxa(Species == 's__Blautia_obeum') %>% psmelt() %>%
  select(-any_of(other_levels),-OTU,-Abundance) %>%
  unique() %>% 
...
psm = psm %>% left_join(met) %>% left_join(crp) %>% left_join(long)
...
ggsave('../Results/Supplementary Figures/Fig. 4d - Metabolites vs Disease Severity - Univar.png',height=3.5,width=8)

# Multivar
ggarrange(plotlist = list(p))
ggsave('../Results/Supplementary Figures/Fig. 4d - Metabolites vs Disease Severity - Multivar.png',height=3.5,width=8)
```
...
writexl::write_xlsx(list('Univar' = spearman,
                    ...
                    '../Results/Supplementary Data/Progression/Metabolites vs Baseline Severity Stats.xlsx')
```