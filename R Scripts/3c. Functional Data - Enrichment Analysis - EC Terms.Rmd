...
u1 = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/EC_Stats.xlsx',sheet='univar') %>% 
  combine_da_results(alpha=pval_cutoff,stat='p_val')
length(unique(u1$Species)) == nrow(u1) # True if no conflicting results

m1 = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/EC_Stats.xlsx',sheet='multivar') %>% 
...
```{r 1.1.1}
relevant_ec = brite$EC[brite$L3 == '1.1.1'] %>% unique()
# How many hits are positive and negative?
pwy_pos = allsig[allsig$EC %in% relevant_ec &
                                allsig$association_dir=='Positive',] %>% unique()
pwy_neg = allsig[allsig$EC %in% relevant_ec &
                                allsig$association_dir=='Negative',] %>% unique()
```

```{r 6.3.4}
relevant_ec = brite$EC[brite$L3 == '6.3.4'] %>% unique()
# How many hits are positive and negative?
pwy_pos = allsig[allsig$EC %in% relevant_ec &
                                allsig$association_dir=='Positive',] %>% unique()
pwy_neg = allsig[allsig$EC %in% relevant_ec &
...
```{r 6.3}
relevant_ec = brite$EC[brite$L2 == '6.3'] %>% unique()
# How many hits are positive and negative?
pwy_pos = allsig[allsig$EC %in% relevant_ec &
                                allsig$association_dir=='Positive',] %>% unique()
pwy_neg = allsig[allsig$EC %in% relevant_ec &
...
```{r Save Results}
writexl::write_xlsx(list('EC Stats' = all_brite),'../Results/Supplementary Data/Enrichment Analysis/EC.xlsx')
```
