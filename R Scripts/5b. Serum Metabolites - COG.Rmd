...
```{r Prep dataset}
# Load sig pwy
temp = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/COG Enzymes_Stats.xlsx',sheet='univar') %>% 
  filter(q_val<0.05) %>% group_by(Species) %>% add_count() %>% filter(n>1) %>% 
  select(Species,association_dir) %>% unique
temp2 = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/COG Enzymes_Stats.xlsx',sheet='multivar') %>%
...
ggsave('../Results/Supplementary Figures/Fig. 3cd - COG vs Metabolites - Univar.png',height=20,width=11)
```
...
ggarrange(plotlist = list(p1,p2),ncol = 2, nrow=1,widths=c(4,1.5))
ggsave('../Results/Supplementary Figures/Fig. 3cd - COG vs Metabolites - Multivar.png',height=20,width=11)
```
