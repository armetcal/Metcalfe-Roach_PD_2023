...
u1 = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/KO_Stats.xlsx',sheet='univar') %>% 
  combine_da_results(alpha=pval_cutoff,stat='p_val')
length(unique(u1$Species)) == nrow(u1) # True if no conflicting results

m1 = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/KO_Stats.xlsx',sheet='multivar') %>% 
...
```{r Mitochondrial Biogenesis}
var = c('Mitochondrial biogenesis [BR:ko03029]')
brite_cat = brite$L2[brite$L1 %in% var] %>% unique()
f.L2 = run_fisher(level='L2',brite_cat,min_terms = 4)
...
all_mito = rbind(f.L2,f.L3,f.L4)
all_mito_pvalsig = all_mito %>% filter(pval<0.05)

# Ugly code that adds the hierarchy information
brite_trunc = brite %>% select(L1:L4) %>% filter(L1==var) %>% unique()
all_mito_info = all_mito_pvalsig %>% 
  filter(Test != '') %>% 
  mutate(rownum = row_number()) %>% 
  pivot_wider(names_from = Level, values_from = Test) %>% 
  mutate(L3 = apply(.,1,function(x){
...
all_mito_qvalsig = all_mito_info %>% filter(qval<0.05)

relevant_ko = brite$all[brite$L4 == 'Mitochondrial biogenesis [BR:ko03029] | Mitochondrial DNA transcription, translation, and replication factors | Mitochondrial transcription and translation factors | Other mitochondrial DNA transcription and translation factors'] %>% unique()
mito_sig_ko = allsig[allsig$KO %in% relevant_ko & allsig$association_dir=='Positive',] %>%
  unique()
```
...
writexl::write_xlsx(list('BRITE' = all_brite,'KEGG' = all_kegg,
                         'Mitochondria' = all_mito, 'Ribosomes' = all_ribo),
                    '../Results/Supplementary Data/Enrichment Analysis/KO.xlsx')

writexl::write_xlsx(list('BRITE' = all_brite_qvalsig,'KEGG' = all_kegg_qvalsig, 
                         'Mitochondria' = all_mito_qvalsig,'Ribosomes' = all_ribo_qvalsig,
                         'Mito Sig KO' = mito_sig_ko, 'Translation Sig KO' = all_translation_sigko),
                    '../Results/Supplementary Data/Enrichment Analysis/KO - Sig Only.xlsx')
```
