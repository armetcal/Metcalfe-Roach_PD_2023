...
```{r Prep dataset}
# Load sig pwy
temp = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/EC_Stats.xlsx',sheet='univar') %>% 
  filter(q_val<0.05) %>% group_by(Species) %>% add_count() %>% filter(n>1) %>% 
  select(Species,association_dir) %>% unique
temp2 = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/EC_Stats.xlsx',sheet='multivar') %>%
...
ggarrange(plotlist = list(p1,p2),ncol = 2, nrow=1,widths=c(4,2))
ggsave('../Results/Supplementary Figures/Fig. 3cd - EC vs Metabolites - Univar.png',height=17,width=8.5)
```
...
ggarrange(plotlist = list(p1,p2),ncol = 2, nrow=1,widths=c(4,2))
ggsave('../Results/Supplementary Figures/Fig. 3cd - EC vs Metabolites - Multivar.png',height=17,width=8.5)
```
