...
ps = readRDS('../Reference Files/taxonomy_phyloseq.rds') %>% 
  ...
  left_join(risk)
df = ps %>% select(Sample,Status,Sex,bristol,logcoffeeperday,smokeprev_or_before_pd,contains('s__'))
...
fun = readRDS('../Reference Files/functional_phyloseq.rds') %>% .[['pwy']] %>% 
  ...
  pivot_wider(names_from = Species, values_from = Abundance) %>% 
  left_join(risk)
df2 = fun %>% select(Sample,Status,Sex,bristol,logcoffeeperday,smokeprev_or_before_pd,all_of(sig))
...
sig = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/KO_Stats.xlsx',sheet='multivar') %>%
...
ko = readRDS('../Reference Files/functional_phyloseq.rds') %>% .[['ko']] %>% 
  ...
  pivot_wider(names_from = Species, values_from = Abundance) %>% 
  left_join(risk)
df3 = ko %>% select(Sample,Status,Sex,bristol,logcoffeeperday,smokeprev_or_before_pd,all_of(sig))

# COG
sig_uni = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/COG Enzymes_Stats.xlsx',sheet='univar') %>% 
  filter(q_val<0.05) %>% group_by(Species) %>% add_count() %>% filter(n>1) %>% 
  select(Species,association_dir) %>% unique
sig = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/COG Enzymes_Stats.xlsx',sheet='multivar') %>%
  filter(Variable=='StatusPD', Species %in% sig_uni$Species) %>% 
  group_by(DA) %>% mutate(q_val = p.adjust(p_val,method='BH')) %>% ungroup %>% 
  filter(q_val<0.05) %>% group_by(Species) %>% add_count() %>% filter(n>1) %>% pull(Species) %>% unique
...
df4 = eggnog %>% select(Sample,Status,Sex,bristol,logcoffeeperday,smokeprev_or_before_pd,all_of(sig))

# EC
sig_uni = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/EC_Stats.xlsx',sheet='univar') %>% 
  filter(q_val<0.05) %>% group_by(Species) %>% add_count() %>% filter(n>1) %>% 
  select(Species,association_dir) %>% unique
sig = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/EC_Stats.xlsx',sheet='multivar') %>%
  filter(Variable=='StatusPD', Species %in% sig_uni$Species) %>% 
  group_by(DA) %>% mutate(q_val = p.adjust(p_val,method='BH')) %>% ungroup %>% 
  filter(q_val<0.05) %>% group_by(Species) %>% add_count() %>% filter(n>1) %>% pull(Species) %>% unique
...
df5 = ec %>% select(Sample,Status,Sex,bristol,logcoffeeperday,smokeprev_or_before_pd,all_of(sig))
...
df6 = met %>% select(Sample,Status,Sex,bristol,logcoffeeperday,smokeprev_or_before_pd,any_of(ts7$Metabolite.Identity))
# Filter out NAs
...
```{r Status}
write.csv(df %>% filter(!is.na(bristol),!is.na(smokeprev_or_before_pd), !is.na(logcoffeeperday)),
          '../Reference Files/FOR RF - Status - Taxonomy.csv',row.names=F)
write.csv(df2 %>% filter(!is.na(bristol),!is.na(smokeprev_or_before_pd), !is.na(logcoffeeperday)),  
          '../Reference Files/FOR RF - Status - MetaCyc.csv',row.names=F)
write.csv(df3 %>% filter(!is.na(bristol),!is.na(smokeprev_or_before_pd), !is.na(logcoffeeperday)),
          '../Reference Files/FOR RF - Status - KO.csv',row.names=F)
write.csv(df4 %>% filter(!is.na(bristol),!is.na(smokeprev_or_before_pd), !is.na(logcoffeeperday)), 
          '../Reference Files/FOR RF - Status - COG.csv',row.names=F)
write.csv(df5 %>% filter(!is.na(bristol),!is.na(smokeprev_or_before_pd), !is.na(logcoffeeperday)), 
          '../Reference Files/FOR RF - Status - EC.csv',row.names=F)
write.csv(df6 %>% filter(!is.na(bristol),!is.na(smokeprev_or_before_pd), !is.na(logcoffeeperday)), 
...
```{r Progression}
write.csv(long_all %>% filter(!is.na(bristol),!is.na(logcoffeeperday)) %>% select(-smokeprev_or_before_pd),
          '../Reference Files/FOR RF - Progression - Taxonomy.csv',row.names=F)
write.csv(long_all2 %>% filter(!is.na(bristol),!is.na(logcoffeeperday)) %>% select(-smokeprev_or_before_pd), 
          '../Reference Files/FOR RF - Progression - MetaCyc.csv',row.names=F)
write.csv(long_all3 %>% filter(!is.na(bristol),!is.na(logcoffeeperday)) %>% select(-smokeprev_or_before_pd),
          '../Reference Files/FOR RF - Progression - KO.csv',row.names=F)
write.csv(long_all4 %>% filter(!is.na(bristol),!is.na(logcoffeeperday)) %>% select(-smokeprev_or_before_pd), 
          '../Reference Files/FOR RF - Progression - COG.csv',row.names=F)
write.csv(long_all5 %>% filter(!is.na(bristol),!is.na(logcoffeeperday)) %>% select(-smokeprev_or_before_pd), 
          '../Reference Files/FOR RF - Progression - EC.csv',row.names=F)s
write.csv(long_all6 %>% filter(!is.na(bristol),!is.na(logcoffeeperday)) %>% select(-smokeprev_or_before_pd), 
          '../Reference Files/FOR RF - Progression - Metabolites.csv',row.names=F)
write.csv(long_all6 %>% filter(!is.na(bristol),!is.na(logcoffeeperday)) %>% select(-smokeprev_or_before_pd) %>% select(-contains('Unknown_')),
