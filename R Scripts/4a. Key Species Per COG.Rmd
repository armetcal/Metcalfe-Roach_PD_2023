...
sig_multi = readxl::read_xlsx('../Results/Supplementary Data/Differential Abundance/COG Enzymes_Stats.xlsx',sheet='multivar') %>%
...
```{r}
brite = read.csv('../Reference Files/COG_key.csv',colClasses = 'character')
...
stats.edit = stats %>% 
  separate(col = Pathway, into=c('COG','Function'),sep=': ')

# More formatting
gt_tbl = stats.edit %>% select(COG, everything()) %>%
  ...
  arrange(qval_Univariate) %>% arrange(-Sig) %>% arrange(Species) %>% arrange(COG)
